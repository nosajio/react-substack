...
describe('Builder fns', () => {
  describe('newParagraph', () => {
    ...
    it('passes innerHTML to Node', () => {
      const node = newParagraph(el);
      expect(node?.children).toBe(innerValue);
    });
  ...
  describe('newBlockquote', () => {
    const innerValue =
      '<p><strong>Hello</strong></p><h2>world</h2><p>hello again</p>';
    let el: Element;
    ...
    it('returns undefined when the blockquote element is empty', () => {
      const emptyBlockquote = document.createElement('blockquote');
      const node = newBlockquote(emptyBlockquote);
      expect(node).toBeUndefined();
    });

    it('contains all child nodes', () => {
      const node = newBlockquote(el);
      node?.children?.[0].type === NodeType.PARAGRAPH;
      node?.children?.[1].type === NodeType.HEADING;
      node?.children?.[2].type === NodeType.PARAGRAPH;
    });
  });

  describe('newList', () => {
    const innerValue = `<li><p><strong>Transport options:</strong> Uber, Taxi, Coach.</p></li><li><p><strong>Time:</strong> you've got 3hrs until boarding.</p></li><li><p><strong>Luggage:</strong> one large suitcase and a small bag.</p></li><li><p><strong>Assistance:</strong> one helpful member of hotel staff speaks English.</p></li>`;
    let el: HTMLUListElement;
    let elOrdered: HTMLOListElement;
    let node: ListNode;
    let orderedNode: ListNode;

    beforeAll(() => {
      el = document.createElement('ul');
      elOrdered = document.createElement('ol');
      el.innerHTML = innerValue;
      elOrdered.innerHTML = innerValue;
      node = newList(el) as ListNode;
      orderedNode = newList(elOrdered) as ListNode;
    });

    it('returns undefined when a UL is passed with no LIs', () => {
      const emptyListEl = document.createElement('ul');
      emptyListEl.innerHTML = '<p>hello world</p>';
      const nonListNode = newList(emptyListEl);
      expect(nonListNode).toBeUndefined();
    });

    it('parses all list items as ListItemNodes', () => {
      expect(node.items).toHaveLength(4);
      node.items.forEach((n) => expect(n.type).toBe(NodeType.LI));
    });

    it('has list items with correct NodeType(s)', () => {
      node.items.forEach((n) =>
        expect(n.children[0].type).toBe(NodeType.PARAGRAPH),
      );
    });

    it('marks ordered lists', () => {
      expect(node.ordered).toBeFalsy();
      expect(orderedNode.ordered).toBeTruthy();
    });

    it('parses nested lists', () => {
      const nested = document.createElement('ul');
      nested.innerHTML =
        '<li><p>hello <a href="https://example.com">world</a></p><ul><li><p>hello again</p></li><li><p>and again</p></li></ul></li>';
      const nestedNode = newList(nested) as ListNode;
      expect(nestedNode.items[0].children[0].type).toBe(NodeType.PARAGRAPH);
      expect(nestedNode.items[0].children[1].type).toBe(NodeType.LIST);
    });
  });
...
describe('parseItemElement', () => {
  ...
  it('parses cover', () => {
    expect(post.cover).toBe(
      'https://substackcdn.com/image/fetch/h_600,c_limit,f_auto,q_auto:good,fl_progressive:steep/https%3A%2F%2Fbucketeer-e05bbc84-baa3-437e-9518-adb32be77984.s3.amazonaws.com%2Fpublic%2Fimages%2F139556c2-f22d-4283-9372-87348c44d8d5_2000x800.jpeg',
    );
  });
  ...
  it('parses link', () => {
    expect(post.link).toBe(
      'https://nosaj.substack.com/p/why-twitter-deserves-to-live',
    );
  });
...
describe('parseBody', () => {
  ...
  it('parses paragraphs correctly', () => {
    ...
    expect((parsedBody[1] as ParagraphNode)?.children).toBe(
  ...
  it('parses headings correctly', () => {
    const node = parsedBody[6] as HeadingNode;
    expect(node.type).toBe(NodeType.HEADING);
    expect(node.level).toBe(2);
    expect(node.children).toBe('🌟 Conviction');
  });

  it('parses lists correctly', () => {
    const node = parsedBody[3] as ListNode;
    expect(node.type).toBe(NodeType.LIST);
    expect(node.ordered).toBeFalsy();
    expect(node.items).toHaveLength(4);
  });

  it('parses horizontal rules correctly', () => {
    const node = parsedBody[17] as HrNode;
    expect(node.type).toBe(NodeType.HR);
  });

  it('contains all expected elements, and ignores subscribe blocks', () => {
    expect(parsedBody.length).toBe(21);
  });
  ...
  it('generated bodyHTML string contains all NodeTypes', () => {
    ...
    expect(parsedHTML.includes('<blockquote>')).toBeTruthy();
  });
});
