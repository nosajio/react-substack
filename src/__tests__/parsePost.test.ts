  ...
  describe('newList', () => {
    ...
    it('has list items with correct NodeType(s)', () => {
      node.items.forEach((n) =>
        expect(n.contents[0].type).toBe(NodeType.PARAGRAPH),
      );
    });
    ...
    it('parses nested lists', () => {
      ...
      expect(nestedNode.items[0].contents[0].type).toBe(NodeType.PARAGRAPH);
      expect(nestedNode.items[0].contents[1].type).toBe(NodeType.LIST);
    });
  });
...
describe('parseBody', () => {
  ...
  it('contains all expected elements, and ignores subscribe blocks', () => {
    expect(parsedBody.length).toBe(20);
  });
